...
function dialogDescription(action: ProjectLifecycleAction, title: string, recovery?: string | null) { const descriptions: Record<ProjectLifecycleAction,string> = { pause: "Pause everyday work while keeping the stage, proof, validation path, and history intact.", resume: "Return this project to active work and make it your current focus. No AI call or XP award occurs.", complete: "Use this when the intended founder journey is finished. Evidence, Value Proof, XP attribution, and lessons remain.", archive: "Remove this project from everyday views while preserving its complete history.", abandon: "Preserve what you learned and remove the project from active work. This is a neutral, reversible decision.", restore: "Return the project to active work. Its previous founder stage is retained and its validation path is reviewed deterministically.", soft_delete: `“${title}” will leave ordinary views and remain recoverable for 30 days. Existing XP totals stay intact.`, permanent_delete: `This permanently removes “${title}” and its private project content. XP totals remain, but project-linked XP metadata is sanitized. ${recovery ? `Recovery was available until ${formatRecoveryDate(recovery)}.` : ""}` }; return descriptions[action]; }
