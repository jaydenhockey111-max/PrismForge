...
export default async function ProjectDetailPage({
  ...
  searchParams: Promise<{ message?: string; error?: string; section?: string }>;
}) {
  ...
  await logBetaEvent({ userId: profile.id, projectId: project.id, eventName: "next_best_action_loaded", source: "project_detail", metadata: { route: nextMove.routeKey, action_type: nextMove.actionType, status }, throttleSeconds: 15 * 60 });
  await logBetaEvent({ userId: profile.id, projectId: project.id, eventName: "next_move_viewed", source: "project_detail", metadata: { route: nextMove.routeKey, action_type: nextMove.actionType, status }, throttleSeconds: 15 * 60 });
  ...
  if (activeSection === "today" || activeSection === "project") {
    await Promise.all([
      ...
      logBetaEvent({ userId: profile.id, projectId: project.id, eventName: "core_loop_next_action_viewed", source: "project_detail", metadata: { section: activeSection, route: nextMove.routeKey, action_type: nextMove.actionType, cohorts }, throttleSeconds: 15 * 60 }),
    ]);
  }
      ...
      {activeSection === "today" && (
        <>
          ...
          <div className="mt-6 grid items-start gap-6 xl:grid-cols-2 [&>*]:mt-0">
            ...
            />
            <WhatChangedCard nextMove={nextMove} projectId={project.id} />
          </div>
          ...
          <div className="surface p-6 sm:p-8">
            <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-start">
              <div>
                ...
                <p className="mt-3 max-w-4xl text-sm leading-6 text-ink/60">The durable project definition, read from left to right. Today remains the place for one Next Move.</p>
              </div>
        ...
        <>
          ...
          <ReportSectionCard kicker="Market validation" title="Validate before you build">
            ...
            <div className="grid gap-5 md:grid-cols-2">
              <ListBlock title="Search questions to test" items={report.marketValidation.searchDemandAssumptions} />
              <ListBlock title="Audience-response questions" items={report.marketValidation.socialDemandAssumptions} />
              <ListBlock title="Likely alternatives to research" items={report.marketValidation.existingAlternatives} />
              <ListBlock title="Pain hypotheses for interviews" items={report.marketValidation.userComplaints} />
            </div>
          </ReportSectionCard>
        </>
      )}
      ...
      <details className="mt-8 rounded-[2rem] border border-ink/10 bg-white p-5 shadow-card sm:p-6">
        ...
        <div className="mt-6 border-t border-ink/10 pt-6">
          ...
          <div className="project-report-rail mt-4 grid gap-6">
            <div className="project-report-panel grid gap-6">
            ...
            <div className="project-report-panel"><ReportSectionCard kicker="Market validation" title="Validate before you build">
              ...
              <div className="mt-5 rounded-2xl border border-moss/15 bg-lime/25 p-5">
                <p className="font-bold text-moss">Positioning hypothesis</p>
  ...
  return (
    <section id="next-move" className="mt-6 overflow-hidden rounded-[2rem] border border-violet/20 bg-ink p-6 text-white shadow-card sm:p-8">
      <div className="grid gap-7 lg:grid-cols-[minmax(0,1fr)_22rem] lg:items-start">
        <div>
          <p className="text-xs font-black uppercase tracking-[.18em] text-lime">Your Next Move</p>
          <h2 className="mt-4 max-w-4xl font-display text-4xl font-semibold leading-[1.02] tracking-tight text-white sm:text-5xl">{nextMove.title}</h2>
          <p className="mt-5 max-w-3xl text-base leading-7 text-white/70"><span className="font-black text-white">Why:</span> {nextMove.why}</p>
          <div className="mt-6 grid gap-3 sm:grid-cols-2">
            <div className="rounded-2xl bg-white/10 p-4"><p className="text-xs font-black uppercase tracking-[.14em] text-lime">Done when</p><p className="mt-2 text-sm font-semibold leading-6 text-white/80">{nextMove.doneWhen}</p></div>
        ...
        <aside className="rounded-[1.5rem] border border-white/10 bg-white/5 p-5">
          ...
          <div className="mt-5 border-t border-white/10 pt-5">
            ...
            <p className="mt-3 text-xs font-semibold text-white/45">{nextMove.effort} · {actionTypeLabel(nextMove.actionType)}</p>
          </div>
        </aside>
      </div>
    </section>
...
function actionTypeLabel(value: NextMove["actionType"]) {
  ...
  if (value === "ai_executable") return "AI-executable after approval";
...
function evidenceChangeSummary(experiment?: ProjectValidationExperiment, assumptionStatus?: string | null) {
  ...
  return `The latest ${evidenceLabel} is saved, but the result remains inconclusive. The recommendation now focuses on collecting a clearer signal.`;
}
