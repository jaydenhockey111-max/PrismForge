...
import { ArrowUpRight, CheckCircle2, FolderKanban, HelpCircle, MessageCircle, Rocket, Sparkles, Trophy, Users } from "lucide-react";
...
import { BUSINESS_TYPE_LABELS, PROJECT_STATUS_LABELS, PROJECT_STATUSES } from "@/lib/founder-os/helpers";
...
export default async function DashboardPage({
  ...
  return (
    <div>
      ...
      <section>
        <div className="prism-shell relative overflow-hidden rounded-[2rem] border border-ink/10 bg-ink p-8 text-white shadow-glow">
          <div className="absolute -right-24 -top-24 size-80 rounded-full bg-gold/20 blur-3xl" />
          <div className="absolute -bottom-28 left-1/3 size-72 rounded-full bg-violet/25 blur-3xl" />
          <div className="relative">
            <div className="flex items-center gap-2 text-sm font-black uppercase tracking-[.16em] text-gold">
              <Sparkles className="size-4" />
              Start here
            </div>
            <h1 className="mt-3 max-w-4xl font-display text-4xl font-semibold tracking-tight sm:text-6xl">Welcome back, {name}. Prove one idea with one real action.</h1>
            <p className="mt-4 max-w-2xl leading-7 text-white/70">
              PrismForge turns an idea into a validation plan, outreach assets, proof tracking, and one clear next move.
            </p>
            <div className="mt-7 flex flex-wrap gap-3">
              <ButtonLink href={nextMove.href} className="bg-gold text-ink hover:bg-white">{nextMove.cta}</ButtonLink>
              {!isFirstTime && <ButtonLink href="/projects" variant="secondary" className="border-white/30 bg-white text-ink hover:bg-gold">View projects</ButtonLink>}
      ...
      <details className="mt-8 rounded-[2rem] border border-ink/10 bg-white p-5 shadow-card"><summary className="cursor-pointer font-black text-violet">Open activity overview</summary><section className="mt-5 grid gap-4 border-t border-ink/10 pt-5 md:grid-cols-2 xl:grid-cols-4">
      ...
      <section className="mt-10">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div>
            <p className="text-sm font-black uppercase tracking-[.16em] text-moss">Projects</p>
            <h2 className="mt-2 font-display text-3xl font-semibold">Pick up where you left off</h2>
          </div>
        ...
        {safeRecentProjects.length === 0 ? (
          ...
          <div className="mt-6 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
            {safeRecentProjects.map((project: any) => (
              <Link key={project.id} href={`/projects/${project.id}`} className="group rounded-[1.75rem] border border-ink/10 bg-white p-6 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-card">
                ...
                <h3 className="mt-5 font-display text-2xl font-semibold leading-tight text-ink group-hover:text-moss">{project.title}</h3>
...
function StatCard({ icon, label, value, detail, tone }: { icon: React.ReactNode; label: string; value: string | number; detail: string; tone: "sky" | "gold" | "lime" | "violet" }) {
  ...
  return (
    <div className={`rounded-[1.5rem] border p-6 ${tones[tone]}`}>
...
function QuickLink({ href, icon, title, text }: { href: string; icon: React.ReactNode; title: string; text: string }) {
  return (
    <Link href={href} className="group rounded-[1.5rem] border border-ink/10 bg-white p-5 shadow-sm transition hover:-translate-y-1 hover:shadow-card">
